/* a2z Tech Web UI - SQLite backend via /api/ */
const API = '';
const T = {
    en: {
        ...
        unlock_fail: 'Incorrect password', about_blurb: 'Desktop inventory management for a2z Tech.',
        ...
        about_us: 'About Us', about_app_name: 'a2z Tech',
        ...
        scan_to_connect_hint: 'Same Wi‑Fi — open the full a2z Tech app on phone or tablet.',
        copy_url: 'Copy URL'
    },
    ar: {
        ...
        unlock_fail: 'كلمة المرور غير صحيحة', about_blurb: 'نظام إدارة المخزون لـ a2z Tech.',
        ...
        about_us: 'من نحن', about_app_name: 'a2z Tech', about_version: 'الإصدار 1.0.2 Platinum',
        ...
        scan_to_connect_hint: 'نفس الواي فاي — افتح تطبيق a2z Tech بالكامل على الهاتف أو الجهاز اللوحي.',
        copy_url: 'نسخ الرابط'
    }
};
...
async function openQuotationPreview(orderId) {
    try {
        ...
        if (company) company.textContent = (data.companyName || 'a2z Tech').toUpperCase();
...
const DEFAULT_PIECE_UOMS = ['g', 'kg', 'pcs', 'pack', 'box', 'meter', 'liter'];
...
function setProductUomOptions(_byWeight, preferred) {
    ...
    let opts = [{ value: '', label: '—' }, ...getPieceUomOptions()];
...
async function addCustomUom() {
    if (getProductTypeValue() === 'Service') return;
...
function updatePieceStockLabels() {
    ...
    if (stockLabel) stockLabel.textContent = uomVal ? `${tr('col_stock')} (${uomVal})` : tr('col_stock');
    if (minLabel) minLabel.textContent = uomVal ? `${tr('low_level')} (${uomVal})` : tr('low_level');
    if (stockHint) {
...
function updateSellByWeightUI(preferredUom) {
    const isService = getProductTypeValue() === 'Service';
    setProductUomOptions(false, preferredUom);
    const priceCol = document.getElementById('p-price-col-label');
    if (priceCol) priceCol.textContent = tr('col_price');
    const pricesLegend = document.getElementById('p-prices-legend');
    if (pricesLegend) pricesLegend.textContent = tr('prices');
